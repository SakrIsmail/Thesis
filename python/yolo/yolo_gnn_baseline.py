...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
yolo = YOLO("/var/scratch/sismail/models/yolo/runs/bikeparts_gnn_baseline/weights/best.pt").eval()
yolo.to(device)
...
for epoch in range(1, num_epochs+1):
    with EmissionsTracker(log_level="critical", save_to_file=False) as tracker:
        ...
        if macro_f1 > best_macro_f1:
            best_macro_f1 = macro_f1
            no_improve = 0
            torch.save(yolo.state_dict(), "/var/scratch/sismail/models/yolo/yolo_gnn_baseline_model.pth")
        else:
            no_improve += 1
