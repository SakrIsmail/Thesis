...
def evaluate_model(model, loader, device):
    ...
    Args:
        ...
        device (torch.device): Device to run the model on.
    Returns:
        list: A list of dictionaries containing predicted and true missing parts.
    """
    model.eval()
    results = []
    P = model.P
    ...
    with torch.no_grad():
        for imgs, tgts in loader:
            ...
            for tgt, out in zip(tgts, outs):
                ...
                for lbl, miss in zip(
                    tgt["labels"].cpu().tolist(), tgt["is_missing"].cpu().tolist()
                ):
                    idx0 = lbl - 1
                    if miss == 1:
                        vec_true[idx0] = 1
...
def part_level_evaluation(results, all_parts):
    ...
    Args:
        results (list): List of dictionaries containing predicted and true missing parts.
    ...
    for j, part_name in enumerate(all_parts):
        ...
        table.append(
            [part_name, f"{acc:.3f}", f"{prec:.3f}", f"{rec:.3f}", f"{f1:.3f}"]
        )
...
class FocalFastRCNNPredictor(FastRCNNPredictor):
    """
    A modified FastRCNNPredictor that uses Focal Loss for classification.
    Args:
...
class HallucinationFasterRCNN(nn.Module):
    """
    A Faster R-CNN model with a modified predictor for hallucination detection.
    Args:
        all_parts (list): List of all parts to detect.
        trainable_backbone_layers (int): Number of trainable layers in the backbone.
        alpha_miss (float): Alpha parameter for Focal Loss.
    ...
    def __init__(
        ...
        self.model.roi_heads.box_predictor = FocalFastRCNNPredictor(
            in_features, num_classes=total_classes
        )
    ...
    def forward(self, images, targets=None):
        ...
        Args:
            images (list): List of images as tensors.
            targets (list, optional): List of target dictionaries containing boxes and labels.
        Returns:
            dict: If training, returns a dictionary of loss values. If evaluating, returns predictions.
        """
        if self.training:
            new_targets = []
            for t in targets:
            ...
            pooled_feats = self.model.roi_heads.box_roi_pool(
                feats, proposals, img_shapes
            )
            ...
            for out in outs:
                ...
                final.append(
                    {
                        "boxes_missing": (
                            torch.stack(miss_boxes)
                            if miss_boxes
                            else torch.zeros((0, 4), device=boxes.device)
                        ),
                        "scores_missing": (
                            torch.tensor(miss_scores, device=boxes.device)
                            if miss_scores
                            else torch.tensor([], device=boxes.device)
                        ),
                        "labels_missing": (
                            torch.tensor(miss_labels, device=boxes.device)
                            if miss_labels
                            else torch.tensor([], device=boxes.device)
                        ),
                    }
    ...
    with EmissionsTracker(log_level="critical", save_to_file=False) as tracker:
        ...
        with tqdm(train_loader, unit="batch", desc=f"Epoch {epoch}/{epochs}") as tepoch:
            ...
            with torch.no_grad():
                for imgs, tgts in valid_loader:
                    ...
                    for out, t in zip(outs, tgts):
                        ...
                        for idx, box in enumerate(out["boxes_missing"]):
                            vec[idx % len(vec)] = 1
                        preds.append(vec)
                        trues.append(t["is_missing"].numpy())
            macro_f1 = f1_score(
                np.vstack(trues), np.vstack(preds), average="macro", zero_division=0
            )
            sched.step(macro_f1)
            ...
            if macro_f1 > best_macro_f1:
                ...
                torch.save(
                    model.state_dict(),
                    "/var/scratch/sismail/models/faster_rcnn/fasterrcnn_MobileNet_missing_baseline_model.pth",
                )
            else:
                no_improve += 1
